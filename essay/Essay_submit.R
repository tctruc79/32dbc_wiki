...
# [Essay_submit.R only] checkpoint Part A's trend data before panel_OECD /
# fig1_dat get reused and overwritten by Parts B and C, so the book-export
# section at the end of this script can still rebuild a merged 3-panel figure.
fig1_dat_A <- fig1_dat %>% mutate(part = "Part A", outcome = "ln(CO2 emission intensity)",
                                   mean_value = mean_co2)
...
# [Essay_submit.R only] checkpoint Part B's trend data - see Part A note above.
fig1_dat_B <- fig1_dat %>% mutate(part = "Part B", outcome = "ln(Labor productivity)",
                                   mean_value = mean_lnprod)
...
# [Essay_submit.R only] checkpoint Part C's trend data - see Part A note above.
fig1_dat_C <- fig1_dat %>% mutate(part = "Part C", outcome = "ln(Labor income share)",
                                   mean_value = mean_ls)
...
# ============================================================
# STEP 27b: ASEAN-INTERACTED CHANNEL DECOMPOSITION (Essay extension)
# Answers "which channel drives ASEAN's weaker labor-share response?" by
# adding an AIPI_EM x ASEAN interaction to each channel of STEP 27. By the
# same accounting identity, the interaction betas on wage + employment - VA
# sum EXACTLY to the interaction beta on labor share, so the identity holds
# for the ASEAN deviation as well as the non-ASEAN baseline.
# ============================================================
asean_iso3_c <- c("BRN","KHM","IDN","LAO","MYS","MMR","PHL","SGP","THA","VNM")
chan_as <- chan_dat %>%
    mutate(ASEAN = if_else(country %in% asean_iso3_c, 1L, 0L),
           Ax    = AIPI_EM * ASEAN)

fe_ls_as <- feols(ln_labshare_em ~ AIPI_EM + Ax + ln_gdppc + renew + rd + trade + wgi
                  | pair_id_em + year, data = chan_as, cluster = ~pair_id_em)
fe_w_as  <- feols(ln_wage_em     ~ AIPI_EM + Ax + ln_gdppc + renew + rd + trade + wgi
                  | pair_id_em + year, data = chan_as, cluster = ~pair_id_em)
fe_e_as  <- feols(ln_emp_em      ~ AIPI_EM + Ax + ln_gdppc + renew + rd + trade + wgi
                  | pair_id_em + year, data = chan_as, cluster = ~pair_id_em)
fe_v_as  <- feols(ln_va_em       ~ AIPI_EM + Ax + ln_gdppc + renew + rd + trade + wgi
                  | pair_id_em + year, data = chan_as, cluster = ~pair_id_em)

bi <- function(m, v) unname(coef(m)[v])
cat("[STEP 27b] N =", nrow(chan_as), "| ASEAN obs =", sum(chan_as$ASEAN),
    "| ASEAN countries in Part C:",
    paste(sort(unique(chan_as$country[chan_as$ASEAN == 1])), collapse = ", "), "\n")
cat("[STEP 27b] Non-ASEAN baseline: labshare", round(bi(fe_ls_as,"AIPI_EM"),4),
    "| wage", round(bi(fe_w_as,"AIPI_EM"),4),
    "| emp", round(bi(fe_e_as,"AIPI_EM"),4),
    "| VA", round(bi(fe_v_as,"AIPI_EM"),4), "\n")
cat("[STEP 27b] ASEAN interaction: labshare", round(bi(fe_ls_as,"Ax"),4),
    "| wage", round(bi(fe_w_as,"Ax"),4),
    "| emp", round(bi(fe_e_as,"Ax"),4),
    "| VA", round(bi(fe_v_as,"Ax"),4), "\n")
cat("[STEP 27b] Identity (interaction): wage + emp - VA =",
    round(bi(fe_w_as,"Ax") + bi(fe_e_as,"Ax") - bi(fe_v_as,"Ax"), 4),
    "vs labshare", round(bi(fe_ls_as,"Ax"),4), "(exact)\n")

modelsummary(
    list("(1) FE labshare" = fe_ls_as, "(2) FE avg wage" = fe_w_as,
         "(3) FE employment" = fe_e_as, "(4) FE value added" = fe_v_as),
    coef_map = c("AIPI_EM" = "AI Patent Intensity (ln) [EM]",
                 "Ax"      = "AI Patent Intensity x ASEAN"),
    title    = "Table 27. ASEAN Channel Decomposition of the Labor-Share Response (Emerging-Market Panel)",
    gof_omit = "AIC|BIC|Log|RMSE|F$|Std",
    notes    = c(
        "Channels: ln(labshare) = ln(avg wage) + ln(employment) - ln(VA);",
        "interaction betas in cols (2)+(3)-(4) sum exactly to col (1).",
        "Non-ASEAN EM is the baseline; the interaction is the ASEAN deviation.",
        "SE clustered at pair level. * p<0.10, ** p<0.05, *** p<0.01. Alpha = 0.10."),
    stars  = c("*" = 0.10, "**" = 0.05, "***" = 0.01),
    fmt = 4,
    output = paste0("output/partC/tables/Table27_ASEAN_Channels_", run_ts, ".docx"))
cat("[STEP 27b] Table 27 saved\n")

# ============================================================
# STEP 27c: LEAVE-ONE-ASEAN-COUNTRY-OUT ON THE WAGE-CHANNEL FINDING
# Robustness check on STEP 27b: does any single ASEAN country in the Part C
# EM panel (IDN, MYS, PHL, THA) drive the negative wage-channel interaction?
# ============================================================
asean_in_chan <- sort(unique(chan_as$country[chan_as$ASEAN == 1]))
loo_asean_wage <- lapply(asean_in_chan, function(cc) {
    d <- chan_as %>% filter(country != cc)
    mw  <- tryCatch(feols(ln_wage_em     ~ AIPI_EM + Ax + ln_gdppc+renew+rd+trade+wgi
                          | pair_id_em + year, data=d, cluster=~pair_id_em), error=function(e) NULL)
    mls <- tryCatch(feols(ln_labshare_em ~ AIPI_EM + Ax + ln_gdppc+renew+rd+trade+wgi
                          | pair_id_em + year, data=d, cluster=~pair_id_em), error=function(e) NULL)
    if (is.null(mw) || is.null(mls)) return(NULL)
    tibble(dropped = cc,
           labshare_Ax = round(unname(coef(mls)["Ax"]), 4),
           wage_Ax     = round(unname(coef(mw)["Ax"]), 4),
           n_obs       = nobs(mls))
}) %>% bind_rows()
loo_asean_wage <- bind_rows(
    tibble(dropped = "(none - full sample)",
           labshare_Ax = round(bi(fe_ls_as,"Ax"), 4),
           wage_Ax     = round(bi(fe_w_as,"Ax"), 4),
           n_obs       = nobs(fe_ls_as)),
    loo_asean_wage)
write_csv(loo_asean_wage, paste0("output/partC/tables/Table28_LOO_ASEAN_WageChannel_", run_ts, ".csv"))
cat("[STEP 27c] Leave-one-ASEAN-country-out (wage-channel robustness):\n")
print(as.data.frame(loo_asean_wage))
cat("[STEP 27c] Sign of wage_Ax across all leave-one-out folds:",
    ifelse(all(loo_asean_wage$wage_Ax[-1] < 0), "NEGATIVE IN EVERY FOLD (robust)",
           "sign flips in at least one fold"), "\n")
...
# ============================================================
# BOOK SUBMISSION EXPORTS (Essay_submit.R only - not in Essay_main.R)
# Reuses model objects and Part A/B/C checkpoints already computed above;
# no models are refit here. Produces exactly 5 tables + 3 figures for GS
# Heshmati's book chapter brief (2026-07-22): 6-8k words, 3-5 tables,
# 2-3 figures, 1-2 appendices, black-and-white only, 3-4 decimals.
# Part C (AI Patent Intensity and the Labor Income Share) is the focal
# story; Parts A and B contribute one shared table + one shared figure
# giving the environmental/efficiency "big picture" context.
# ============================================================
cat("\n============================================================\n")
cat("BOOK SUBMISSION EXPORTS starting\n")
cat("============================================================\n")

for (d in c("output/book/tables", "output/book/figures")) {
    if (!dir.exists(d)) dir.create(d, recursive = TRUE)
}

## ---- Book Table 1: sample architecture across Parts A-C (reuses tab_s2) ----
datasummary_df(tab_s2,
    title = "Book Table 1. Sample Architecture and Headline FE-IV Estimates Across Parts A-C",
    notes = "Pairs = country x industry. beta = coefficient on the part's key technology regressor (FE-IV main spec).",
    fmt = 4,
    output = paste0("output/book/tables/BookTable1_SampleArchitecture_", run_ts, ".docx"))
cat("[BOOK] Table 1 saved\n")

## ---- Book Table 2: headline FE-IV across Parts A-C (reuses mod_list) ----
modelsummary(
    mod_list,
    coef_map = c(
        "fit_AIPI"      = "AI Patent Intensity (ln, IV) [OECD]",
    ...
    title    = "Book Table 2. Headline FE-IV Estimates Across Parts A-C: Environment, Efficiency and Distribution (OECD vs EM)",
    gof_omit = "AIC|BIC|Log|RMSE|F$|Std",
    notes    = c(
        "Each column re-estimates the MAIN FE-IV specification of the corresponding part.",
        "Part A: AIPI -> ln CO2 emission intensity. Part B: ln GTI -> ln labor productivity.",
        "Part C: AIPI -> ln labor income share. Samples and IVs differ by design across parts.",
        "SE clustered at pair level. Alpha = 0.10. * p<0.10, ** p<0.05, *** p<0.01."),
    stars  = c("*" = 0.10, "**" = 0.05, "***" = 0.01),
    fmt = 4,
    output = paste0("output/book/tables/BookTable2_HeadlineAcrossParts_", run_ts, ".docx"))
cat("[BOOK] Table 2 saved\n")

## ---- Book Table 3: Part C main results (the book's focal result) ----
suppressWarnings(modelsummary(
    list("(0) Pooled OLS"   = pooled_ols_OECD,
         "(1) FE"           = fe_base_OECD,
         "(2) FE+Interact." = fe_int_OECD,
         ...
         "(4) Diff. GMM"    = sys_gmm_OECD),
    coef_map = coef_labels,
    title    = "Book Table 3. Main Results: AI Patent Intensity and the Labor Income Share (OECD Panel)",
    gof_omit = "AIC|BIC|Log|RMSE|F$|Std",
    notes    = c(
        "Dependent variable: ln(labor income share) = ln(LABR / VALU).",
        "FE-IV: instrument = AIPI_bartik (leave-one-out mean AI patents, other 24 OECD countries, same year).",
        "SE clustered at pair level (Cols 0-3); two-step robust GMM SE (Col 4).",
        "* p<0.10, ** p<0.05, *** p<0.01. Alpha = 0.10."),
    stars  = c("*" = 0.10, "**" = 0.05, "***" = 0.01),
    fmt = 4,
    output = paste0("output/book/tables/BookTable3_PartC_MainResults_", run_ts, ".docx")))
cat("[BOOK] Table 3 saved\n")

## ---- Book Table 4: Part C channel decomposition ----
suppressWarnings(modelsummary(
    list("(a) Employment"   = fe_iv_emp,
         "(b) Average wage" = fe_iv_wage,
         "(c) Value added"  = fe_iv_va),
    coef_map = coef_labels,
    title    = "Book Table 4. Channel Decomposition: Employment, Wage and Value-Added Effects (FE-IV)",
    gof_omit = "AIC|BIC|Log|RMSE|F$|Std",
    notes    = c(
        "Dependent variables: (a) ln(Total employment); (b) ln(Average wage); (c) ln(Value added).",
        "Same FE-IV specification and instrument (AIPI_bartik) as Book Table 3, col (3).",
        "Identity: ln(labor share) = ln(avg. wage) + ln(employment) - ln(value added).",
        "SE clustered at pair level. * p<0.10, ** p<0.05, *** p<0.01. Alpha = 0.10."),
    stars  = c("*" = 0.10, "**" = 0.05, "***" = 0.01),
    fmt = 4,
    output = paste0("output/book/tables/BookTable4_ChannelDecomposition_", run_ts, ".docx")))
cat("[BOOK] Table 4 saved\n")

## ---- Book Table 5: Part C first-stage + key subsample robustness, merged ----
suppressWarnings(modelsummary(
    list("First stage (AIPI ~ AIPI_bartik)" = first_stage,
         "(R1a) High-capital" = fe_iv_hi,
         "(R1b) Low-capital"  = fe_iv_lo),
    coef_map = c(
        "AIPI_bartik" = "AIPI_bartik [instrument]",
        coef_labels),
    title    = "Book Table 5. Instrument Validity and Subsample Robustness (OECD Panel)",
    gof_omit = "AIC|BIC|Log|RMSE|F$|Std",
    notes    = c(
        "First stage: DV = AIPI; instrument = AIPI_bartik (Bartik shift-share, other 24 OECD countries).",
        "R1: FE-IV on high-capital (HighCapital=1) vs low-capital (HighCapital=0) subsamples,",
        "country-specific median threshold.",
        "SE clustered at pair level. * p<0.10, ** p<0.05, *** p<0.01. Alpha = 0.10."),
    stars  = c("*" = 0.10, "**" = 0.05, "***" = 0.01),
    fmt = 4,
    output = paste0("output/book/tables/BookTable5_FirstStageRobustness_", run_ts, ".docx")))
cat("[BOOK] Table 5 saved\n")

## ---- Book Figure 1: merged 3-panel trend (a)(b)(c), B&W ----
## Simple cross-industry average by year and part (not per-industry detail,
## which the standalone Part A/B/C Fig1s already cover) - this is the
## compact "big picture" panel the book format calls for.
trend_book <- bind_rows(
    fig1_dat_A %>% group_by(part, outcome, year) %>%
        summarise(mean_value = mean(mean_value, na.rm = TRUE), .groups = "drop"),
    fig1_dat_B %>% group_by(part, outcome, year) %>%
        summarise(mean_value = mean(mean_value, na.rm = TRUE), .groups = "drop"),
    fig1_dat_C %>% group_by(part, outcome, year) %>%
        summarise(mean_value = mean(mean_value, na.rm = TRUE), .groups = "drop")
)

make_trend_panel <- function(part_name) {
    ggplot(trend_book %>% filter(part == part_name), aes(x = year, y = mean_value)) +
        geom_line(linewidth = 0.8, color = "black") +
        geom_point(size = 1.8, color = "black", shape = 16) +
        labs(x = NULL, y = trend_book$outcome[trend_book$part == part_name][1],
             title = part_name) +
        theme_bw(base_size = 10) +
        theme(plot.title = element_text(size = 10, face = "bold"))
}

p_book_fig1 <- (make_trend_panel("Part A") | make_trend_panel("Part B") | make_trend_panel("Part C")) +
    plot_annotation(tag_levels = "a",
        caption = paste0("Book Figure 1. Panel-Wide Trends by Margin, OECD Panel, 2010-2022.\n",
                          "(a) Environmental margin: mean ln(CO2 emission intensity). ",
                          "(b) Efficiency margin: mean ln(labor productivity). ",
                          "(c) Distributional margin: mean ln(labor income share).\n",
                          "Simple cross-industry average by year; black and white only."),
        theme = theme(plot.caption = element_text(hjust = 0.5, size = 9, face = "italic")))
ggsave(paste0("output/book/figures/BookFig1_TrendsByMargin_", run_ts, ".png"),
       p_book_fig1, width = 11, height = 4.2, dpi = 300)
cat("[BOOK] Figure 1 saved\n")

## ---- Book Figure 2: headline coefficient comparison across parts, B&W ----
## Reuses meta_df from S1; OECD/EM distinguished by point shape (filled vs
## open circle), not color, so it prints correctly in black and white.
p_book_fig2 <- meta_df %>%
    mutate(lo90 = beta - qnorm(0.95) * se,
           hi90 = beta + qnorm(0.95) * se,
           panel_lab = paste0(essay, "\n(", outcome, ")")) %>%
    ggplot(aes(x = sample, y = beta, shape = sample)) +
    geom_hline(yintercept = 0, linetype = "dashed", color = "grey40") +
    geom_errorbar(aes(ymin = lo90, ymax = hi90), width = 0.15, linewidth = 0.7, color = "black") +
    geom_point(size = 3, color = "black", fill = "black") +
    scale_shape_manual(values = c(OECD = 16, EM = 1)) +
    facet_wrap(~panel_lab, scales = "free_y") +
    labs(x = NULL, y = "FE-IV coefficient on technology regressor", shape = NULL,
         caption = paste("Book Figure 2. Headline FE-IV estimates across Parts A-C, OECD (filled circle) vs EM (open circle).",
                         "Error bars = 90% CI (alpha = 0.10); SE clustered at pair level.",
                         "Outcomes and regressors differ across facets - compare within facet only.",
                         sep = "\n")) +
    theme_bw(base_size = 11) +
    theme(legend.position = "bottom", plot.caption = element_text(hjust = 0, size = 9))
ggsave(paste0("output/book/figures/BookFig2_CrossPartsCoefs_", run_ts, ".png"),
       p_book_fig2, width = 9, height = 5.5, dpi = 300)
cat("[BOOK] Figure 2 saved\n")

## ---- Book Figure 3: Part C channel decomposition, B&W (reuses channel_df) ----
p_book_fig3 <- ggplot(channel_df, aes(x = channel, y = coef, fill = sign_col)) +
    geom_col(color = "black", linewidth = 0.4, width = 0.6) +
    geom_errorbar(aes(ymin = coef - ci, ymax = coef + ci), width = 0.15, color = "black") +
    geom_hline(yintercept = 0, color = "black", linewidth = 0.4) +
    geom_vline(xintercept = 3.5, linetype = "dashed", color = "grey40", linewidth = 0.4) +
    geom_text(aes(label = sprintf("%+.4f", coef),
                   y = coef + ifelse(coef >= 0, ci + 0.006, -ci - 0.014)),
               fontface = "bold", size = 3.3) +
    scale_fill_manual(values = c(pos = "grey75", neg = "grey35", main = "grey55"), guide = "none") +
    labs(x = NULL, y = "FE-IV coefficient on AI Patent Intensity (ln)",
         caption = paste0("Book Figure 3. Channel Decomposition and Accounting Identity (FE-IV), Part C.\n",
                           "Wage + Employment - Value added = Labor share (identity check, gap = 0.0000).")) +
    theme_bw(base_size = 11) +
    theme(plot.caption = element_text(hjust = 0.5, size = 9, face = "italic"))
ggsave(paste0("output/book/figures/BookFig3_ChannelDecomposition_", run_ts, ".png"),
       p_book_fig3, width = 8, height = 5, dpi = 300)
cat("[BOOK] Figure 3 saved\n")

cat("\n============================================================\n")
cat("BOOK SUBMISSION EXPORTS complete: 5 tables + 3 figures in output/book/\n")
cat("Reminder: check final essay word count (target 6,000-8,000 words).\n")
cat("============================================================\n")
