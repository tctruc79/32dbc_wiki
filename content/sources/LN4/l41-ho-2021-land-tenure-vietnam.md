...
Keywords: Land tenure, Privatization, Economic development, Southeast Asia, Vietnam.<br><span
class="en">**Author**: Hoang-Anh Ho (University of Economics Ho Chi Minh City). JEL: O11, P48,
...
quốc năm 1993 của Việt Nam — cấp **land-use certificates** cho đất nông nghiệp.<br><span
class="en">**Summary (paraphrase)**: An influential line of cross-country research — led by
...
reform, which issued **land-use certificates** for agricultural land.</span>

## Research Questions

Private property rights đối với đất tác động thế nào lên phát triển kinh tế cấp xã (commune) ở
Việt Nam, sau cải cách ruộng đất 1993?<br><span class="en">How do private property rights over
land affect commune-level economic development in Vietnam following the 1993 land reform?</span>
...
phát triển kinh tế, nên nếu bỏ sót sẽ overestimate tác động của land tenure.<br><span
class="en">Following Besley (1995) and Besley & Ghatak (2010) — the same theory that
...
nhận độ tin cậy của proxy này.<br><span class="en">The Commune Module of the 2004 VHLSS
(Vietnam Household Living Standards Survey), a random sample of ~2,205 out of 8,000 rural
communes. The private-land-tenure variable = the share of agricultural land area holding
land-use certificates (sample mean: 74.41%). The dependent variable is **nighttime light
intensity** (logged, plus 0.01) from NOAA satellite data for 2005 — used because Vietnam has no
commune-level GDP data; cross-checked against 2002 VHLSS per-capita consumption, yielding a
Pearson correlation of 0.73 (p=0.000), confirming the reliability of this proxy.</span>
...
khác nhau của **coefficient of proportionality (δ)** giữa 0 và 1, và R từ 0,65 đến 0,80.<br><span
class="en">**Two empirical models**: (1) **Panel data** (1992, pre-reform, vs. 2005,
post-reform) — fixed-effects vs. random-effects, with a Hausman test (rejecting the
random-effects null at p=0.000) — this handles **time-invariant** confounders. (2) **2004
cross-section** combined with the **Oster (2019)** method — estimating the sensitivity of the
estimated coefficient to UNOBSERVED confounders, based on how much the coefficient shifts as
observed confounders are added progressively, adjusted for the R² of the "true
data-generating process." This is an alternative to instrumental variables (the author
acknowledges being unable to find a credible IV) — it yields a coefficient range under
different scenarios for the **coefficient of proportionality (δ)** between 0 and 1, and R from
0.65 to 0.80.</span>
...
- **Panel data (Table 2)**: random-effects cho hệ số 0,012*** (1% land-use certificates tăng →
  ...
  theo thời gian.<br><span class="en">**Panel data (Table 2)**: random-effects gives a
  coefficient of 0.012*** (a 1% increase in land-use certificates → a 1.2% increase in
  nighttime light); fixed-effects lowers this to 0.009*** — indicating the presence of
  time-invariant confounders.</span>
- **Cross-section (Table 3)**: hồi quy đơn biến (cột 1) cho hệ số <span class="stat">0,017</span>
  ...
  certificates → chỉ còn 0,6% tăng nighttime light. R² đầy đủ = 0,617.<br><span class="en">
  **Cross-section (Table 3)**: the univariate regression (column 1) gives a coefficient of
  ...
  the coefficient falls sharply to <span class="stat">0.006</span>*** — a 1% increase in
  land-use certificates now yields only a 0.6% increase in nighttime light. Full R² =
  0.617.</span>
- **Oster (2019) bias-adjustment (Table 4)**: ở kịch bản lạc quan nhất (δ=0,1, R=0,65), hệ số
  ...
  tenure.<br><span class="en">**Oster (2019) bias-adjustment (Table 4)**: under the most
  optimistic scenario (δ=0.1, R=0.65), the coefficient remains 0.006***. Under the **most
  conservative** scenario (δ=0.9, R=0.80), the coefficient **is not different from zero** —
  meaning unobserved confounders COULD fully explain the observed effect of private land
  tenure.</span>
...
Nam (đặc biệt Đồng bằng sông Cửu Long) thuận lợi hơn cho thành công kinh tế của cải cách 1993.<br><span
class="en">Results hold up across a range of checks — the intensive margin (communes with
positive luminosity only: coefficient falls to 0.003), nighttime light per capita, nighttime
light growth, district-clustered standard errors. There is a marked North–South difference: the
...
tradition of private land tenure, which made the 1993 reform more economically effective
there.</span>
...
giao dịch đất cao** so với các nước Đông Á khác (Childress 2004).<br><span class="en">The
effect of private land tenure on rural economic development in Vietnam is **modest**. The main
explanations: (i) **lingering insecurity** — the state can still reclaim land-use certificates
(typically with below-market compensation) upon expiry; an estimated ~4% of households had land
reclaimed during 2006–2012 (Markussen & Tarp 2014); (ii) **high taxes and time costs** for land
transactions relative to other East Asian countries (Childress 2004).</span>
...
rights (không chỉ sự tồn tại hình thức) mới quyết định độ lớn tác động kinh tế thực tế.<br><span
class="en">The lesson is valuable not only for Vietnam's future land reform but also for other
developing countries considering agricultural land privatization: the degree of completeness
of property rights (not merely their formal existence) determines the magnitude of the real
economic effect.</span>
...
- **Liên kết cross-lecture quan trọng với LN2**: paper này xây dựng trực tiếp trên lập luận
  ...
  được **kiểm định thực nghiệm bằng dữ liệu vi mô Việt Nam** ở LN4. Xem thêm [[institutions]].<br><span
  class="en">**An important cross-lecture link to LN2**: this paper builds directly on the
...
- **Tương phản quan trọng với AJR (2001)**: trong khi nghiên cứu xuyên quốc gia của AJR tìm thấy
  ...
  cấp địa phương.<br><span class="en">**A meaningful contrast with AJR (2001)**: whereas AJR's
  cross-country research finds a LARGE effect of private property rights on development, Ho
  (2021) finds a **modest** effect that could fall to zero under a conservative scenario —
  suggesting cross-country results may overstate the effect when local-level confounders cannot
  be controlled for.</span>
- **Phương pháp luận đáng chú ý cho ôn thi**: kỹ thuật Oster (2019) là một cách tiếp cận thay
  ...
  [[l42-do-2023-land-consolidation-vietnam]] (PSM-DD).<br><span class="en">**A methodological
  point worth noting for the exam**: the Oster (2019) technique is an alternative to IV when no
  valid instrument can be found — worth comparing with
  [[l46-hastuti-2025-climate-labor-mobility-indonesia]] (a real IV) and
  [[l42-do-2023-land-consolidation-vietnam]] (PSM-DD).</span>
- **Hàm ý chính sách Việt Nam**: cải cách ruộng đất tương lai nên hướng tới quyền sở hữu đất
  vĩnh viễn thay vì land-use certificate có thời hạn, và giảm thuế/chi phí giao dịch đất.<br><span
  class="en">**Policy implications for Vietnam**: future land reform should move toward
  permanent land ownership rather than term-limited land-use certificates, and reduce
  land-transaction taxes/costs.</span>
...
- Cùng lecture (LN4): [[l42-do-2023-land-consolidation-vietnam]] — cả hai đều về thể chế đất đai
  ...
  — hai kênh khác nhau.<br><span class="en">Same lecture (LN4):
  [[l42-do-2023-land-consolidation-vietnam]] — both concern Vietnamese land institutions, but L41
  addresses property rights/tenure security while L42 addresses land fragmentation/consolidation —
  two different channels.</span>
