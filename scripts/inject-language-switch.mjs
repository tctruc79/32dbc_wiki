...
// (public/ va public/en/) ma khong can biet dang build cay nao.
...
<script>(function(){function setup(){var link=document.getElementById("language-switch-link");if(!link)return;var siteRoot=document.body.dataset.basepath||"";var path=window.location.pathname;var rel=siteRoot&&path.indexOf(siteRoot)===0?path.slice(siteRoot.length):path;var enMatch=rel.match(/^\\/en(\\/.*|)$/);if(enMatch){link.href=siteRoot+(enMatch[1]||"/");link.textContent="🇻🇳 Tiếng Việt";link.title="Chuyển sang bản song ngữ (VI)";}else{link.href=siteRoot+"/en"+rel;link.textContent="🇬🇧 English";link.title="Switch to full-English version";}}setup();document.addEventListener("nav",setup);document.addEventListener("render",setup);})();</script>
`
...
async function main() {
  ...
  for (const file of files) {
    ...
    if (html.includes(SNIPPET_MARKER)) continue
